    ...
    for row in rows:
        try:
            ...
            if row.find("td", "microplanet1"):
                ...
                planet.coord = Coord(galaxy_p, system_p, pos_p)
            ...
            if player_id == 99999:
                planet = Planet()
                planet.coord = parse_coord(position)
            ...
            planet.updated = get_servertime_now()
            ...
            player.ogameid = planet.player_id
            player.updated = get_servertime_now()
            ...
            if ogevent_e:
                ...
                ogevent.type = 'planet'
                ogevent.updated = get_servertime_now()
            ...
            if alliance_e:
                ...
                alliance.updated = get_servertime_now()
...
def read_galaxy_data(data):
    def check_name(entity, name):
        if entity.name != name:
            prevs = entity.prev_names
            prevs.append(entity.name)
            entity.prev_names = prevs
            entity.name = name
    ...
    if alliance:
        ...
        if alliance_entity:
            ...
            alliance_entity.updated = alliance.updated
    ...
    old_planet = db.query(Planet).filter_by(_coord = planet._coord).first()
    ...
    if not old_planet:
        ...
        old_planet.updated = planet.updated
        if planet.status == 'vacant' and old_planet.status == 'colonized':
            old_planet.status = 'destroyed'
            db.update(old_planet)
            db.add(planet)
            planet_entity = planet
        elif planet.status == 'colonized' and old_planet.status == 'vacant':
            db.delete(old_planet)
            db.add(planet)
            planet_entity = planet
        elif planet.status == 'colonized' and old_planet.status == 'colonized':
            if planet.player_id == old_planet.player_id:
                old_planet.name = planet.name
                db.update(old_planet)
                planet_entity = old_planet
            else:
                old_planet.status = 'destoryed'
                db.merge(old_planet)
                db.add(planet)
                planet_entity = planet
        else:
...
#        planet_entity = planet
    ...
    datas['player'] = player_entity
    datas['alliance'] = alliance_entity
